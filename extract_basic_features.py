...
Windows + torchcodec 에러 완전 회피 버전
"""
...
def process_one_file(audio_dir, wav_name):
    ...
    print(f"[PROCESS] {wav_name}")

    vocals_path = separate_vocals(wav_path)

    if not os.path.exists(vocals_path):
        print(f"[SKIP] vocals.mp3 not found")
        return
